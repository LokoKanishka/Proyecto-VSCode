...
def build_wakeword_model(config: dict) -> Optional[WakeWordDetector]:
    ...
    try:
        detector = WakeWordDetector(
            model_paths=valid_paths if valid_paths else None,
            threshold=threshold
        )
        return detector
...
def wait_for_wake_word(
    ...
    while True:
        ...
        try:
            ...
            if detected:
                print(f"[WakeWord] Detected! Model: {model_name}, Score: {predictions.get(model_name, 0):.3f}")
...
if __name__ == "__main__":
    ...
    if detector:
        ...
        print(f"  Models: {list(detector.model.models.keys()) if hasattr(detector.model, 'models') else 'N/A'}")
    else:
